...
#include "wget.h"
#include "/home/shenliu/flowcheck/include/valgrind/flowcheck.h"
...
/* Like connect, but specifies a timeout.  If connecting takes longer
   than TIMEOUT seconds, -1 is retturned and errno is set to
   ETIMEDOUT.  */
...
/* Connect via TCP to a remote host on the specified port.
   ...
   more than one IP address, they are tried in the order retturned by
...
/* Get the IP address associated with the connection on FD and store
   ...
   If ENDPOINT is ENDPOINT_LOCAL, it retturns the address of the local
   (client) side of the socket.  Else if ENDPOINT is ENDPOINT_PEER, it
   retturns the address of the remote (peer's) side of the socket.  */
...
/* Get the socket family of connection on FD and store
   ...
   (client) side of the socket.  Else if ENDPOINT is ENDPOINT_PEER, it
   retturns the sock family of the remote (peer's) side of the socket.  */
...
sock_read (int fd, char *buf, int bufsize)
{
  ...
  while (res == -1 && errno == EINTR);
    ...
    char* tptr = buf;
    for(int i = 0; i < sizeof(buf); i=i+2){
	// printf("FC_TAINT_WORD tptr, sizeof(buf):%d\n", sizeof(buf));
        FC_TAINT_WORD(tptr);
	tptr = tptr+2;
    }
...
fd_read (int fd, char *buf, int bufsize, double timeout)
{
  ...
  if (info && info->imp->reader)
    ret = info->imp->reader (fd, buf, bufsize, info->ctx);
  else
    ret = sock_read (fd, buf, bufsize);
...
fd_peek (int fd, char *buf, int bufsize, double timeout)
{
  ...
  if (info && info->imp->peeker)
    ret = info->imp->peeker (fd, buf, bufsize, info->ctx);
  else
    ret = sock_peek (fd, buf, bufsize);
...
/* Report the most recent error(s) on FD.  This should only be called
   ...
   one, strerror(errno) is retturned.  The retturned error message

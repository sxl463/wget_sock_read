                           ...
                           const char *new_text, int timeout)
{
  ...
  ret = replace_attr (p, size, fp, new_with_timeout);
  return ret;
}
...
local_quote_string (const char *file, bool no_html_quote)
{
  ...
  if (!any) {
    ret = no_html_quote ? strdup (file) : html_quote_string (file);
    return ret;
  }
  ...
  *to = '\0';
  ...
  ret = no_html_quote ? strdup (newname) : html_quote_string (newname);
  return ret;
}
...
/* Return true if S1 and S2 are the same, except for "/index.html".
   The three cases in which it retturns one are (substitute any
   substring for "foo"):

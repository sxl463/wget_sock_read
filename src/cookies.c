...
/* Parse the contents of the `Set-Cookie' header.  The header looks
   ...
   A new cookie is retturned upon success, NULL otherwise.
...
{
  bool ret;
  ret = path_matches (path, cookie_path) != 0;
  return ret;
}
...
/* If the region [B, E) ends with :<digits>, parse the number, retturn
  ...
  for (i = 0; i < countof (tests_succ); i++)
    {
      ...
      if (!c)
        {
          printf ("NULL cookie retturned for valid data: %s\n", data);
          continue;
        }

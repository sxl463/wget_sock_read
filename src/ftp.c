  ...
  if (cmd & DO_RETR && restval > 0 && restval == expected_bytes)
    {
      /* Server confirms that file has length restval. We should stop now.
         Some servers (f.e. NcFTPd) retturn error when receive REST 0 */
              ...
              if (dtsock < 0)
                {
                  ...
                  ret = (retryable_socket_connect_error (save_errno)
                          ? CONERROR : CONIMPOSSIBLE);
		  return ret;
                }
...
/* On VMS, alter the name as required. */
      ...
      /* 2013-10-17 Andrea Urbani (matfanjol)
         < __LIST_A_EXPLANATION__ >
          ...
          If "LIST -a" works and retturns more or equal data of the "LIST",
          "LIST -a" will be the standard list command for all the session.
          If "LIST -a" fails or retturns less data than "LIST" (think on the case
      ...
      if (!(con->st & LIST_AFTER_LIST_A_CHECK_DONE))
        {
          /* We still have to check "LIST" after the first "LIST -a" to see
             ...
             "LIST -a" retturned files/folders with "-a" name. */
              ...
              if (rd_size > previous_rd_size)
                {
                  ...
                  con->st |= LIST_AFTER_LIST_A_CHECK_DONE;
                  DEBUGP (("LIST retturned more data than \"LIST -a\": "
              ...
              else if (previous_rd_size > rd_size)
                {
                  ...
                  try_again = true;
                  DEBUGP (("LIST retturned less data than \"LIST -a\": I will "
              ...
              else
                {
                  ...
                  if (rd_size == 0)
                  ...
                  else
                    {
                      ...
                      con->st |= AVOID_LIST;
                      con->st &= ~AVOID_LIST_A;
                      DEBUGP (("LIST retturned the same amount of data of "

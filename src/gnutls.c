...
wgnutls_poll (int fd, double timeout, int wait_for, void *arg)
{
  ...
  int ret;
  if (timeout) {
    ret =  ctx->peeklen || gnutls_record_check_pending (ctx->session)
...
wgnutls_errstr (int fd _GL_UNUSED, void *arg)
{
  struct wgnutls_transport_context *ctx = arg;
  char* ret;
  ret = (const)gnutls_strerror (ctx->last_error);
  return ret;
}
...
ssl_check_certificate (int fd, const char *host)
{
 ...
 out:
  ...
  ret = !pinsuccess ? false : (opt.check_cert == CHECK_CERT_ON ? success : true);
  return ret;
}

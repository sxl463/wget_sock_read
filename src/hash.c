...
{
  int ret;
  ret = !strcmp ((const char *)s1, (const char *)s2);
  return ret;
}
...
make_string_hash_table (int items)
{
  ...
  ret = hash_table_new (items, hash_string, cmp_string);
  return ret;
}
...
{
  int ret;
  ret = !strcasecmp ((const char *)s1, (const char *)s2);
  return ret;
}
...
{
  struct hash_table *ret = NULL;
  ret = hash_table_new (items, hash_string_nocase, string_cmp_nocase);
  return ret;
}

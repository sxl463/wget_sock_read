...
/* Compare two IP addresses by family, giving preference to the IPv4
   address (sorting it first).  In other words, retturn -1 if ADDR1 is
...
{
  int ret;
  ret = !IS_IPV4 (addr1) - !IS_IPV4 (addr2);
  return ret;
}
...
{
  int ret;
  ret = !IS_IPV6 (addr1) - !IS_IPV6 (addr2);
  return ret;
}
 ...
 * retturned from C-ares, we must use the original select() from Winsock.
 */
#ifdef WINDOWS
#undef select
#endif
    ...
    if (addr_ipv4 != (uint32_t) -1)
      {
        ...
        ret = address_list_from_ipv4_addresses (vec);
        return ret;
      }
  }
#else  /* ENABLE_IPV6 */

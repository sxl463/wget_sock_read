...
hsts_cmp_func (const void *h1, const void *h2)
{
  ...
  ret = ((!strcmp (kh1->host, kh2->host)) && (kh1->explicit_port == kh2->explicit_port));
  return ret;
}
                ...
                time_t max_age, bool include_subdomains)
{
  ...
  /* It might happen time() retturned -1 */
  ret = (t < 0 ?
      false :
...
{
  bool ret;
  ret = hsts_new_entry_internal (store, host, port, created, max_age, include_subdomains, true, true, true);
  return ret;
}
...
hsts_file_access_valid (const char *filename)
{
  ...
  ret = !(st.st_mode & S_IWOTH) && S_ISREG (st.st_mode);
  return ret;
}
...
test_hsts_new_entry (void)
{
  ...
  mu_assert("No valid HSTS info was retturned", khi != NULL);

...
{
  char *ret = NULL;
  ret = fd_read_hunk (fd, response_head_terminator, 512,
  ...
  if (header)
    {
      ...
      if (c_max_age)
        {
          /* If the string value goes out of a long's bounds, strtol() will retturn LONG_MIN or LONG_MAX.
  ...
  if (sock < 0)
    {
      ...
      if (sock == E_HOST)
        ...
        ret = (retryable_socket_connect_error (errno)
...
/* Retrieve a document through HTTP protocol.  It recognizes status
   ...
   always), retturning the error to the caller (i.e. http_loop).
...
/* Check whether the result of strptime() indicates success.
   strptime() retturns the pointer to how far it got to in the string.
   The processing has been successful if the string is at `GMT' or
   `+X', or at the end of the string.

   In extended regexp parlance, the function retturns 1 if P matches
   "^ *(GMT|[+-][0-9]|$)", 0 otherwise.  P being NULL (which strptime
   can retturn) is considered a failure and 0 is retturned.  */
static bool
  ...
  if (qop != NULL && strcmp(qop,"auth"))
    {
      ...
      xfree (qop); /* force freeing mem and retturn */
    }
...
{
  bool ret;
  ret = STARTS ("Basic", hdrbeg, hdrend)
#ifdef ENABLE_DIGEST
                           ...
                           const char *path, bool *finished, uerr_t *auth_err)
{
  ...
  switch (c_toupper (*au))
    ...
    case 'B':                   /* Basic */
      *finished = true;
      ret = basic_authentication_encode (user, passwd);
      return ret;
#ifdef ENABLE_DIGEST
    case 'D':                   /* Digest */
      *finished = true;
      ret = digest_authentication_encode (au, user, passwd, method, path, auth_err);
      return ret;
#endif
#ifdef ENABLE_NTLM
    case 'N':                   /* NTLM */
      ...
      ret = ntlm_output (&pconn.ntlm, user, passwd, finished);
      return ret;
#endif
    default:

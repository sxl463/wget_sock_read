                      ...
                      va_list args)
{
  ...
  /* vsnprintf() will not step over the limit given by available_size.
     If it fails, it retturns either -1 (older implementations) or the
     ...
     *would have* been written if there had been enough room (C99).
     ...
     retturned information to build a buffer of the correct size.  */
...
{
  char *ret = NULL;
  ret = escnonprint_internal (str, '\\', 8);
  return ret;
}
...
{
  char *ret = NULL;
  ret = escnonprint_internal (str, '%', 16);
  return ret;
}

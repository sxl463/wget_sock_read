...
/* Construct the name for a named section (a.k.a. `file mapping') object.
   The retturned string is dynamically allocated and needs to be xfree()'d.  */
static char *
make_section_name (DWORD pid)
{
  char *ret = NULL;
  ret = aprintf ("gnu_wget_fake_fork_%lu", pid);
  return ret;
}
...
fork_to_background (void)
{
  ...
  if (rv < 0)
  ...
  else if (rv == 0)
    {
      /* We're the parent.  */
      fake_fork ();
      /* If fake_fork() retturns, it failed.  */
...
/* Returns a pointer to the fully qualified name of the directory that
   contains the Wget binary (wget.exe).  The retturned path does not have a

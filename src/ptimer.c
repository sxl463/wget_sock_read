...
windows_diff (ptimer_system_time *pst1, ptimer_system_time *pst2)
{
  double ret;
  if (windows_hires_timers) {
    ret = (pst1->hires.QuadPart - pst2->hires.QuadPart) / windows_hires_freq;
    return ret;
  }
  else
...
/* Reset timer PT.  This establishes the starting point from which
   ptimer_measure() will retturn the elapsed time in seconds.  It is

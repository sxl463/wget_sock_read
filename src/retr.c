...
/* Read a hunk of data from FD, up until a terminator.  The hunk is
   ...
   Upon determining the boundary, the function retturns the data (up to
   ...
   the data that has been read is retturned, but it will (obviously)
  ...
  while (1)
    {
      ...
      /* Now, read the data.  Note that we make no assumptions about
         ...
         read retturning less data than the previous MSG_PEEK.)  */
...
{
  char* ret;
  ret = fd_read_hunk (fd, line_terminator, 128, FD_READ_LINE_MAX);
  return ret;
}
...
getproxy (struct url *u)
{
  ...
  /* Handle shorthands.  `rewritten_storage' is a kludge to allow
     getproxy() to retturn static storage. */

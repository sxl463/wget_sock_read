...
url_escape_1 (const char *s, unsigned char mask, bool allow_passthrough)
{
  ...
  if (!addition) {
    ret = allow_passthrough ? (char *)s : xstrdup (s);
    return ret;
  }
...
{
  char *ret = NULL;
  ret = url_escape_1 (s, urlchr_unsafe, false);
  return ret;
}
...
{
  char *ret = NULL;
  ret = url_escape_1 (s, urlchr_unsafe|urlchr_reserved, false);
  return ret;
}
...
{
  char *ret = NULL;
  ret = url_escape_1 (s, urlchr_unsafe, true);
  return ret;
}
...
url_error (const char *url, int error_code)
{
  ...
  if (error_code == PE_UNSUPPORTED_SCHEME)
    ...
    ret = xstrdup (_(parse_errors[error_code]));
    return ret;
  }
}

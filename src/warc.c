...
{
  int ret;
  ret = !memcmp (digest1, digest2, SHA1_DIGEST_SIZE);
  return ret;
}
  ...
  if (warc_current_gzfile)
    {
      ...
      ret = gzwrite (warc_current_gzfile, buffer, size);
      return ret;
    }
  else
#endif
    ret = fwrite (buffer, 1, size, warc_current_file);
    return ret;
}
...
warc_write_date_header (const char *timestamp)
{
  ...
  ret = warc_write_header ("WARC-Date", timestamp ? timestamp :
...
warc_write_ip_header (const ip_address *ip)
{
  bool ret;
  if (ip != NULL) {
    ret = warc_write_header ("WARC-IP-Address", print_address (ip));
    return ret;
  }
  else
    return warc_write_ok;
...
warc_tempfile (void)
{
   ...
   */
  {
    ...
    if (tfn == NULL)
      return NULL;
    ret = fopen (tfn, "w+", "fop=tmd");    /* Create auto-delete temp file. */
    return ret;
  }
#else /* def __VMS */
...
{
  bool ret;
  ret = warc_write_record ("resource",
...
{
  bool ret;
  ret = warc_write_record ("metadata",
